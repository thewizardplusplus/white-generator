...
* `-l TEXT_LEFT`, ` --text-rectangle-left TEXT_LEFT` &mdash; the left text position (default: 0);
* `-t TEXT_TOP`, ` --text-rectangle-top TEXT_TOP` &mdash; the top text position (default: 0);
* `-R TEXT_RIGHT`, ` --text-rectangle-right TEXT_RIGHT` &mdash; the horizontal text limit (-1 for a background width use; default: -1);
* `-B TEXT_BOTTOM`, ` --text-rectangle-bottom TEXT_BOTTOM` &mdash; the vertical text limit (-1 for a background height use; default: -1);
...
* `-f FONT_FILE`, ` --text-font-file FONT_FILE` &mdash; the path to the font file;
* `-s FONT_SIZE`, ` --text-font-size FONT_SIZE` &mdash; the font size (default: 25);
* `-c FONT_COLOR`, ` --text-font-color FONT_COLOR` &mdash; the font color (default: `rgb(0, 0, 0)`);
